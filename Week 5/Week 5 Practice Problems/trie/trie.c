...
bool check(char* word)
{
    ...
    {
        index[i] = tolower(word[i]) - 97;

        // if cursor is pointing to NULL means the letter after this sequence of letters
        // does not exist (ie word does not exist).
        if (cursor->children[index[i]] == NULL)
        {
            break;
        }

        // need to do this to move the cursor to the child node
        // where if its a final valid letter, is_word is true. As
        // "is_word attribute of the child node to true" per the
        // implementation notes.
        cursor = cursor->children[index[i]];

        // checks for is_word for the final letter.
        if ((i == length - 1) && (cursor->is_word))
        {
            return true;
        }

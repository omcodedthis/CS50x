...
def main():
    ...
    # TODO: Checks database for matching profiles. The variable matches_found is the number of matches for
    # a single person respectively. The nested for-loop goes through each row of database (len(database) is
    # the int value of the number of rows) & compares the values of each DNA sequence (j+1 to exclude name)
    # to the str_chains list. If the current index of str_chains and the current index of the list in the
    # database list  are the same, means there is a match of the current STR chain and matches_found is
    # increased by 1. If the matches_found equals the length str_chains, meaning all STRs have a match for the,
    # current individual, database[i][j=0], it is this person's DNA & their name is printed, and program exits.
    # If not, the matches_found is set back to zero for the next individual, for the next row of the
    # database. If after iterating through each list (ie contents of a row) in the database list & a full
    # match is not found, "No Match" is printed.
    matches_found = 0
